...
int main(void)
{
    ...
    sintyoku = 0;
    while(count < max_pear){
        sintyoku = next_jadge(s);
        printf("sintyoku:%d\n", sintyoku);
...
//    scanf("%d %d", &sidex, &sidey);
    ...
    if((s.str[n - 2][n - 2] == s.str[n - 1][n - 1]) && (max_pear - count <= 2)){
        printf("3.Hi\n");
//        printf("count:%d\n", count);
        sousa[0][0] = 0;
        sousa[0][1] = 0;
        ...
        while((zahyou[i][0] != x_left || zahyou[i][1] != y_left) && i < Shikou){
            ...
            if(zahyou[i - 1][0] == x_left){ //x座標が一致している場合
                ...
                if(max == 1){
                    printf("zahyou y:%d %d\n", zahyou[i - 1][1], y_left);
                    max = zahyou[i - 1][1] - y_left + 1;
                    if(n - zahyou[i - 1][0] >= max){
                        printf("4-1.Hello\n");
                        printf("max:%d\n", max);
                        sousa[i][0] = zahyou[i - 1][0];
                        sousa[i][1] = zahyou[i - 1][1] - max + 1;
                        ...
                        printf("4-2.Hello\n");
                        sousa[i][0] = zahyou[i - 1][0];
                        sousa[i][1] = zahyou[i - 1][1] - (n - zahyou[i - 1][0]) + 1;
                        sousa[i][2] = n - zahyou[i - 1][0];
                        zahyou[i][0] = zahyou[i - 1][0];
                        zahyou[i][1] = zahyou[i - 1][1] - (n - zahyou[i - 1][0]) + 1;
                    }
                    printf("zahyou:%d %d\n", zahyou[i][0], zahyou[i][1]);
                }
                else{
                    if(a <= n - zahyou[i - 1][1] - 1){
                        printf("4-3.Hello\n");
                        sousa[i][0] = zahyou[i - 1][0];
                        sousa[i][1] = zahyou[i - 1][1];
                        sousa[i][2] = a;
        ...
        while((zahyou[i][0] != x_left || zahyou[i][1] != y_left) && i < Shikou){
            ...
            if(zahyou[i - 1][0] == x_left){ //x座標が一致している場合
                ...
                if(zahyou[i - 1][1] == y_left){
                    ...
                    max = zahyou[i - 1][1] - y_left;
                    if(zahyou[i - 1][0] - x_left > max){ //条件が間違っている?　処理の都合上、正解を導いてしまうことがある
                        printf("3-2-1.Hello\n");
                        sousa[i][0] = zahyou[i - 1][0] - max;
                        sousa[i][1] = zahyou[i - 1][1] - max;
                        sousa[i][2] = max + 1;
                        zahyou[i][0] = zahyou[i - 1][0] - max;
                        zahyou[i][1] = zahyou[i - 1][1];
                        printf("max: %d 3-2-1: %d %d %d\n", max, sousa[i][0], sousa[i][1], sousa[i][2]);
                    }
                    else{
                        printf("3-2-2.Hello\n");
                        sousa[i][0] = x_left;
...
int next_jadge(struct data s){
    ...
    for(i = 0; i < n; i++){
        for(j = 0; j < n; j++){
            if(s.str[i][j] == s.str[i + 1][j] && n - i <= 2){
                count++;
            }
            else if(s.str[i][j] == s.str[i][j + 1]){
                count++;
                j++;
            }
            else{
